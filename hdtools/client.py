...
def get_cookie_header():
    if "HDTOOLS_COOKIE" in os.environ:
        ...
        config.debug(f"Using raw cookie: {cookie}")
        return cookie
        ...
        cookie = f"{name}={value}"
        config.debug(f"Using split cookie: {cookie}")
        return cookie
    else:
...
def get_headers():
    ...
    config.debug(f"Request headers: {headers}")
    return headers

def search_user(username):
    url = f"{BASE_URL}/srv/feed/dynamic/rest/Search/{username}"
    config.debug(f"GET {url}")
    response = requests.get(url, headers=get_headers())

    try:
        response.raise_for_status()
        config.debug(f"Response status: {response.status_code}")
